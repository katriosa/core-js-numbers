...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorALength = Math.sqrt(x1 * x1 + y1 * y1);
  const vectorBLength = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(dotProduct / (vectorALength * vectorBLength));
}
...
function toNumber(value, def) {
  ...
  return Number.isNaN(transformation) ? def : transformation;
}
...
function getFibonacciNumber(index) {
  ...
  return index <= 1 ? index : fibonacciArr[index];
}
/**
...
function getSumOfDigits(num) {
  ...
  const sum = arrayOfStrings.reduce((acc, cur) => +acc + +cur);
  return sum;
}
...
function getCountOfOddNumbers(number) {
  ...
  return Math.floor(absoluteNum / 2) + (absoluteNum % 2);
}
